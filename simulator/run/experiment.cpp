...
void printQueueStatistics(Topology *topo) {
    ...
    std::cout
        ...
        << "% Utilization "	<< utilization / params.bandwidth * 100 << "% " << dst_utilization / params.bandwidth * 100
        << "%\n";
}
...
void run_experiment(int argc, char **argv, uint32_t exp_type) {
    ...
    if (
		...
		|| params.flow_type == FASTPASS_MEAN_FLOW
		|| params.flow_type == FASTPASS_PREDICTION_FLOW
		|| params.flow_type == FASTPASS_BUFFER_FLOW) {
    ...
    } else if (params.flow_type == D3_FLOW || params.flow_type == D3_MEAN_FLOW|| params.flow_type == D3_PREDICTION_FLOW){
    	topology = new D3Topology(params.num_hosts, params.num_agg_switches, params.num_core_switches, params.bandwidth, params.queue_type);
    }
    else {
    ...
    FlowGenerator *fg;
	if (params.use_exact_traces) {
		fg = new TraceFlowGenerator(num_flows, topology, params.trace_location);
		fg->make_flows();
	}
	if (params.use_exact_distribution) {
		fg = new TraceCDFGenerator(num_flows, topology, params.trace_location);
		fg->make_flows();
	}
    ...
    std::cout << " Simulator ended. Execution time: " << duration << " seconds\n";
    //cleanup
    delete fg;
}
