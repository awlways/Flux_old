...
class DCExpParams {
    public:
        ...
        uint32_t mean_predicted_flow_size;

        //Exact traces reproduction
        uint32_t use_exact_traces;
        uint32_t use_exact_distribution;
        std::string trace_location;

        double switching_delay;
        uint32_t number_of_hosts;
        double prediction_improvement = 0;
        double squeezing_factor;
        double host_buffer_size;
