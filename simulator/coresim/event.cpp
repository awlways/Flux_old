...
//        }
    ...
    if (size != 0) {
        flows_to_schedule.push_back(Factory::get_flow(id, time, size, src, dst, params.flow_type, 0, 1));
    }
...
void QueueProcessingEvent::process_event() {
    ...
    if (packet) {
        ...
        double sl = params.switching_delay;
        //double additional_delay = 1e-10;
        queue->queue_proc_event = new QueueProcessingEvent(time + sl + td, queue);
        add_to_event_queue(queue->queue_proc_event);
        queue->busy_events.push_back(queue->queue_proc_event);
        if (next_hop == NULL) {
            Event* arrival_evt = new PacketArrivalEvent(time + sl + td + pd, packet);
            ...
            if (params.cut_through == 1) {
                ...
                queuing_evt = new PacketQueuingEvent(time + sl + td + pd, packet, next_hop);
            }
...
void FlowFinishedEvent::process_event() {
    ...
    if (slowdown < 1.0 && slowdown > 0.95) {
        slowdown = 1.0;
    }
//    if (slowdown < 1.0) {
//        std::cout<< this->flow->id << " bad slowdown " << 1e6 * flow->flow_completion_time
//        	<< " " << topology->get_oracle_fct(flow) << " " << slowdown << " " << this->flow->size << "\n";
//    }
//    assert(slowdown >= 1.0);
    ...
    if (print_flow_result()) {
        ...
        std::cout
			...
			<< 1000000.0 * flow->deadline << " "
			<< 1000000.0 * (flow->deadline - flow->flow_completion_time) << " "
            << std::endl;
