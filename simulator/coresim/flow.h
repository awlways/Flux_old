...
class Flow {
    public:
        Flow(uint32_t id, double start_time, uint32_t size, Host *s, Host *d, uint32_t predicted_flow_size = 0);
        ...
        virtual void send_ack(Packet* received_packet, uint32_t seq, std::vector<uint32_t> sack_list);
        virtual void receive_ack(Ack* ack_packet, uint32_t ack, std::vector<uint32_t> sack_list);
