...
Flow::Flow(uint32_t id, double start_time, uint32_t size, Host *s, Host *d, uint32_t predicted_flow_size) {
    this->id = id;
    this->mss = params.mss;
    ...
    this->cwnd_mss = params.initial_cwnd*mss;
    this->max_cwnd = params.max_cwnd*mss;
    this->finished = false;
    ...
    //set deadline - from 2x to 20x more time than necessary
    //2*13230/1000000.0
    double median = 9.3/100000;
    int multiplier = 4 + rand() % 20;
    //this->deadline = median + multiplier*(topology->get_oracle_fct(this)/1000000.0);
    this->deadline = (20+((double)rand()/RAND_MAX)*20)/1000;
}
...
void Flow::send_ack(Packet* received_packet, uint32_t seq, std::vector<uint32_t> sack_list) {
...
void Flow::receive_ack(Ack* ack_packet, uint32_t ack, std::vector<uint32_t> sack_list) {
...
void Flow::receive(Packet *p) {
    ...
    if (p->type == ACK_PACKET) {
        Ack *a = (Ack *) p;
        receive_ack(a, a->seq_no, a->sack_list);
    }
    else if(p->type == NORMAL_PACKET) {
    	//std::cerr.precision(10);std::cerr<<"WIN "<<get_current_time()<<" "<<p->seq_no<<std::endl;
...
void Flow::receive_data_pkt(Packet* p) {
    ...
    //if(id==1)std::cerr<<"RECEIVED "<<recv_till<<" "<<last_unacked_seq<<std::endl;
    ...
    send_ack(p, recv_till, sack_list); // Cumulative Ack
}
